![Cover Image](Nivedha_LinkedIn_Banner.jpg)

# Hi there, I'm Nivedha Sivakumar 👋  

🎯 **Data Analyst | AI/ML Practitioner | Educator**  
📍 Based in Coimbatore, India  

---

## 🚀 About Me
- 🔍 Recently upskilled in **Data Analytics & AI/ML**  
- 📊 Passionate about **data visualization, storytelling, and dashboard design**  
- 👩‍🏫 Experienced tutor in **STEM, public speaking, and grammar**  
- 💡 Skilled in **SQL, Python, Power BI, and statistical analysis**  
- 🌱 Currently seeking **entry-level roles in Data Analytics & AI/ML**  

---

## 🛠️ Skills & Tools
- **Languages & Libraries:** Python (Pandas, NumPy, Matplotlib, Scikit-learn), SQL  
- **Visualization & BI:** Power BI, Tableau, Matplotlib, Seaborn  
- **Data Handling:** Data wrangling, validation, workflow automation  
- **Other:** Git, LaTeX (resume formatting), requirement analysis, client collaboration  

---

## 📌 Featured Projects
### 🚖 Uber Rides Power BI Dashboard
- Interactive dashboard with **navigation bars, branded visuals, and KPIs**  
- Analyzed ride demand, revenue trends, and customer behavior  
- [View Repository](#) *(add your repo link here)*  

### 📊 SQL Case Studies
- Complex queries using **joins, constraints, unions, and window functions**  
- Real-world datasets for business insights  
- [View Repository](#)  

### 🤖 AI/ML Experiments
- Predictive modeling & customer segmentation prototypes  
- Focus on **clarity, reproducibility, and business impact**  
- [View Repository](#)  
---

## 🌐 Connect With Me
- [LinkedIn](www.linkedin.com/in/nivedhasivakumar)    
- [Email](nivedha_sivakumar@outlook.com)  

---
✨ *“Turning data into decisions, and insights into impact.”*

